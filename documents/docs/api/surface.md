...
分配给定大小的新 surface。像素缓冲区初始化为零（完全透明的黑色）。`owns` 设为 `1`。
...
创建包装已有像素缓冲区的 surface。`pixels` 必须至少为 `width * height * 4` 字节，格式为预乘 RGBA32。`owns` 设为 `0`，意味着调用 `cg_surface_destroy` 将**不会**释放像素数据--调用者负责其生命周期。
...
减少引用计数。当计数归零时，释放像素缓冲区（若 `owns == 1`）和 surface 结构体。传入 `NULL` 安全。
